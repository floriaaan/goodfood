...
export const PromotionDeleteAlert = ({ closeSheet, id }: { closeSheet: () => void; id: Promotion["id"] }) => {
  ...
  const handleDelete = async () => {
    try {
      ...
      if (!res.ok) throw new Error("Une erreur s'est produite lors de la suppression de la promotion");
      toast({
        className: "p-3",
        children: (
          <div className="inline-flex w-full items-end justify-between gap-2">
            <div className="inline-flex shrink-0 gap-2">
              ...
              <div className="flex w-full grow flex-col">
                <ToastTitle>La promotion a été supprimée avec succès</ToastTitle>
              </div>
            </div>
          </div>
        ),
